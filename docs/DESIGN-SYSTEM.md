...
Cards display at a fixed `width="420"` in `README.md` (down from an earlier `width="600"`, which read oversized on wide desktop screens once cards were switched to single-column). GitHub's `max-width:100%` still shrinks them further on narrow/mobile viewports regardless of this value.
